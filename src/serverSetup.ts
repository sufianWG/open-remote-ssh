...
if [[ ! -f $SERVER_SCRIPT ]]; then
    ...
    # Function to attempt download with a given URL
    download_server() {
        local url=$1
        if [[ ! -z $(which wget) ]]; then
            wget --tries=3 --timeout=10 --continue --no-verbose -O vscode-server.tar.gz "$url" 2>&1
        elif [[ ! -z $(which curl) ]]; then
            curl --retry 3 --connect-timeout 10 --location --show-error --silent --output vscode-server.tar.gz "$url"
        else
    ...
    echo "Attempting to download server from primary URL: $SERVER_DOWNLOAD_URL"
    download_server "$SERVER_DOWNLOAD_URL"
    DOWNLOAD_RESULT=$?

    # If primary download fails, try fallback URLs
    if (( $DOWNLOAD_RESULT > 0 )); then
        echo "Primary download failed, trying fallback URLs..."
        
        # Fallback URL patterns
        VSCODE_URL="https://update.code.visualstudio.com/commit:$DISTRO_COMMIT/server-$PLATFORM-$SERVER_ARCH/$DISTRO_QUALITY"
        VSCODIUM_URL="https://github.com/VSCodium/vscodium/releases/download/$DISTRO_VERSION.$DISTRO_VSCODIUM_RELEASE/vscodium-reh-$PLATFORM-$SERVER_ARCH-$DISTRO_VERSION.$DISTRO_VSCODIUM_RELEASE.tar.gz"
        
        # Try VSCode URL if not already tried
        if [[ "$SERVER_DOWNLOAD_URL" != "$VSCODE_URL" ]]; then
            echo "Trying VSCode fallback URL: $VSCODE_URL"
            download_server "$VSCODE_URL"
            DOWNLOAD_RESULT=$?
        fi
        
        # Try VSCodium URL if VSCode failed and not already tried
        if (( $DOWNLOAD_RESULT > 0 )) && [[ "$SERVER_DOWNLOAD_URL" != "$VSCODIUM_URL" ]]; then
            echo "Trying VSCodium fallback URL: $VSCODIUM_URL"
            download_server "$VSCODIUM_URL"
            DOWNLOAD_RESULT=$?
        fi
    fi

    if (( $DOWNLOAD_RESULT > 0 )); then
        echo "Error: Failed to download server from all attempted URLs"
        echo "Primary URL: $SERVER_DOWNLOAD_URL"
        echo "Please check your network connection and the availability of the server binaries"
        print_install_results_and_exit 1
    fi
...
if(!(Test-Path $SERVER_SCRIPT)) {
    del vscode-server.tar.gz -ErrorAction SilentlyContinue

    # Function to attempt download with a given URL
    function Download-Server($url) {
        try {
            $REQUEST_ARGUMENTS = @{
                Uri=$url
                TimeoutSec=20
            ...
            Invoke-RestMethod @REQUEST_ARGUMENTS -ErrorAction Stop
            return $true
        }
        catch {
            return $false
    ...
    "Attempting to download server from primary URL: $primaryUrl"
    $downloadSuccess = Download-Server $primaryUrl

    # If primary download fails, try fallback URLs
    if(-not $downloadSuccess) {
        "Primary download failed, trying fallback URLs..."
        
        # Fallback URL patterns
        $vscodeUrl = "https://update.code.visualstudio.com/commit:$DISTRO_COMMIT/server-win32-x64/$DISTRO_QUALITY"
        $vscodiumUrl = "https://github.com/VSCodium/vscodium/releases/download/$DISTRO_VERSION.$DISTRO_VSCODIUM_RELEASE/vscodium-reh-win32-x64-$DISTRO_VERSION.$DISTRO_VSCODIUM_RELEASE.tar.gz"
        
        # Try VSCode URL if not already tried
        if($primaryUrl -ne $vscodeUrl) {
            "Trying VSCode fallback URL: $vscodeUrl"
            $downloadSuccess = Download-Server $vscodeUrl
        }
        
        # Try VSCodium URL if VSCode failed and not already tried
        if((-not $downloadSuccess) -and ($primaryUrl -ne $vscodiumUrl)) {
            "Trying VSCodium fallback URL: $vscodiumUrl"
            $downloadSuccess = Download-Server $vscodiumUrl
        }
    }

    if(-not $downloadSuccess) {
        "Error: Failed to download server from all attempted URLs"
        "Primary URL: $primaryUrl"
        "Please check your network connection and the availability of the server binaries"
        exit 1
    }
